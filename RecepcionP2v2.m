...
%%FINAL_SIGNAL = s1;
eyediagram(FINAL_SIGNAL(START:START+10*mp),2*mp);
%% Diagrama de ojo
FINAL_SIGNAL = FINAL_SIGNAL(START:end)/max(abs(FINAL_SIGNAL(START:end)));

ed = comm.EyeDiagram('SampleRate',Fs,'SamplesPerSymbol',2*mp)
show(ed);
ed(FINAL_SIGNAL(1:Fs));                      %% La señal tiene que ser un vector columna
%% Continuación
%%% graficar las primeras 100, 500 y 1000 muestras
stem(FINAL_SIGNAL(1:1:1000));
vint = round(abs(FINAL_SIGNAL(START:mp:8*mp*1089+START-1)/max(FINAL_SIGNAL(START:8*mp*1089+START-1))));
%%vint(vint <= 0) = 0;
%%
